    ...
    state = { 
     ...
     onDocumentLoadSuccess = ({ numPages }) => {
      ...
      goToNextPage = () =>
        this.setState(state => ({ pageNumber: state.pageNumber + 1 }));
    render() { 
        const links = this.props.match.url==='/Book/Auth'?<SignedInNavbar/>:<Navbar/>;
        const download = this.props.match.url==='/Book/Auth'?<button className="btn-download">Download</button>:<Link to="/signin"><span className="span-download">Sign in to download</span></Link>;
